	...
	cv.addEventListener("touchstart", function (e) {
  		...
  		if (e.timeStamp-lastTap < 300) {
  		    ...
  		    tmout = setTimeout(function() {
  		        sim.@com.falstad.ripple.RippleSim::longPress()();
  		    }, 1000);
  		}
  		lastTap = e.timeStamp;
...
//		setup = (Setup) setupList.elementAt(0);
			...
			while (st.hasMoreTokens()) {
				...
				try {
					if (tint == '$') {
						...
						setWaveType();
						brightnessBar.setValue(Integer.parseInt(st.nextToken()));
	...
	class HardCodedSetup extends Setup {
		private String title;
		private String settings;

		HardCodedSetup(String title, String settings) {
			this.title = title;
